...
- **低收入组**呈正关联（β = +0.85；cluster bootstrap p = 0.065），对逐一剔除稳健（+0.66至+1.02），集中于MDG时代（+1.27, p = 0.038），SDG时代衰减至零（+0.03, p = 0.928）。
- GHE与财务保护改善相关：每增加1个百分点GDP的公共筹资，自付医疗支出相对下降约11%（间接效应 a×b = +0.10；bootstrap 95% CI +0.02至+0.23）。
