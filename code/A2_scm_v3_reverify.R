...
# ---- Load ----
dt <- as.data.table(readRDS("results_pipeline_b/panel_analysis.rds"))
...
plabs[, flabel := sprintf("%s\n(placebo p=%.3f)", Country, Placebo_p)]
plabs[, flabel := gsub(".", "·", flabel, fixed=TRUE)]
...
gap_df$Country <- factor(gap_df$Country, levels=plabs$flabel)
p_vals_text <- paste(sprintf("%s: p=%.3f", sumdt$Country, sumdt$Placebo_p), collapse="; ")

p2 <- ggplot(gap_df, aes(x=year, y=gap, group=id)) +
  geom_line(data=gap_df[gap_df$placebo==1,], color="grey65", linewidth=0.4, alpha=0.7) +
  ...
  theme_pub
ggsave("results/figures/figS6_synth_gaps.pdf", p2, width=170, height=100, units="mm", device=cairo_pdf)
cat("Saved: results/figures/figS6_synth_gaps.pdf\n")
